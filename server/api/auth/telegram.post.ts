...
function getDataCheckString(body: TelegramLegacyAuthBody) {
...
function verifyTelegramAuth(body: TelegramLegacyAuthBody, botToken: string) {
  if (!body.hash || !body.auth_date || !body.id) {
    return false
  }
  ...
  const checkString = getDataCheckString(body)
  const computedHash = createHmac('sha256', secretKey).update(checkString).digest('hex')

  const expected = Buffer.from(body.hash, 'hex')
...
export default eventHandler(async (event) => {
  try {
    ...
    const body = await readBody<TelegramLegacyAuthBody>(event)

    if (!body.id || !body.hash) {
      throw createError({
        statusCode: 400,
        message: 'Missing Telegram authorization payload'
      })
    }
    ...
    const userId = `telegram:${body.id}`
    const fullName = `${body.first_name || ''} ${body.last_name || ''}`.trim()
    const login = body.username || fullName || userId
    const avatarUrl = body.photo_url
    const isAdmin = adminUserIds.includes(userId)
      || adminUserIds.includes(body.id)
      || (body.username ? adminUserIds.includes(body.username) : false)
