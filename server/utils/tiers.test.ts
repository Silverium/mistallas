...
describe('tier limits', () => {
  it('defines expected purchase limits for each tier', () => {
    ...
    expect(getPurchaseLimit('enterprise', defaultLimits)).toBe(10000)
  })
  ...
  it('never limits enterprise tier', () => {
    ...
    expect(isAtLimit('enterprise', 100_000_000, defaultLimits)).toBe(true)
  })

  it('calculates remaining and used percentage safely', () => {
    expect(getRemainingPurchases('free', 150, defaultLimits)).toBe(50)
    expect(getRemainingPurchases('free', 1000, defaultLimits)).toBe(0)

    expect(getPercentageUsed('free', 100, defaultLimits)).toBe(50)
    expect(getPercentageUsed('enterprise', 8000, defaultLimits)).toBe(80)
  })

  it('respects custom limits from config', () => {
    const customLimits = { free: 100, premium: 1000, enterprise: 10000 }
