...
<template>
  <div class="flex flex-col gap-4">
    ...
    <ClientOnly>
      ...
      >
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          >
            Recursos {{ hasOfflineResources ? 'listos' : isCachingOfflineAssets ? 'guardando…' : offlineResourcesStatus
              === 'error' ? 'con error' : 'pendientes' }}
          </UBadge>
          ...
          >
            Datos {{ hasOfflineData ? 'listos' : isSyncing ? 'sincronizando…' : status === 'error' ? 'con error'
              : 'pendientes' }}
          </UBadge>
          ...
          >
            Páginas {{ areOfflinePagesReady ? 'listas' : isPreparingOfflinePages ? 'preparando…' : offlinePagesStatus
              === 'error' ? 'con error' : 'pendientes' }}
          </UBadge>
        </div>
        ...
        >
          No se pudieron guardar todos los recursos base de la app para uso sin conexión. Mantén la app abierta con
          conexión e
          inténtalo de nuevo.
        </p>
        ...
        >
          <UIcon
            ...
            class="animate-spin"
          />
          <span>Preparando páginas offline ("Compras" y "Medidas")…</span>
        </div>
        ...
        <p
          ...
          class="text-xs text-muted"
        >
          Páginas offline listas: "Compras" y "Medidas".
    ...
    <p
      ...
      class="font-medium"
    >
      Hola: ¿Te acuerdas de la talla y modelo de pantalones que necesitas de esa marca que cambia el tallaje incluso
      cuando
      se elije otro color del pantalón?
    </p>
    <p v-if="!loggedIn">
    ...
    >
      No se almacena ninguna información personal de tu cuenta en la base de datos.<br>
    </p>
  </div>
</template>
